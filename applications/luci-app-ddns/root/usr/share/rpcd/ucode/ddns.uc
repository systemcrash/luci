...
let _dateFmt = '';
_dateFmt = (uci.get('ddns', 'global', 'ddns_dateformat') || '%F %R');
_dateFmt = replace(_dateFmt, /%n/g, '<br />'); // Replace '%n' with '<br />'
const dateFormat = replace(_dateFmt, /%t/g, '    '); // Replace '%t' with four spaces
...
function get_date(seconds, format) {
	return replace(trim( popen(`date -d @${seconds} "+${format}" 2>/dev/null`)?.read?.('line') ), '\n', '');
}

// // convert epoch date to given format
// function epoch2date(epoch, format) {
// 	return null; //get_date(epoch, format);
// }
...
const methods = {
	...
	get_ddns_state: {
		call: function() {
			// const _dateFormat = get_dateformat();
			...
			// Crash triggers on the next line on a guinea-pig system
			res['_curr_dateformat'] = get_date(time(), _dateFormat);
			res['_services_list'] = (services_mtime && get_date(services_mtime, _dateFormat)) || 'NO_LIST';
