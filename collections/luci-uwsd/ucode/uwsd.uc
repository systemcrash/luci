...
export function onConnect(conn, protocols) {
	...
	if (!ticketdata) {
		warn(`WS connection: No ticket found in protocols. Available: ${join(', ', protocols)}`);
		conn.close(1002, 'No ticket in subprotocol proposal');
		return;
	}
...
function send_keepalive() {
	for (let conn in uwsd.connections())
		if (conn.protocol() in [ "ws", "wss" ]) {
			// Send keepalive via ws(s) connections only. Sending this via
			// busy http(s) connections results in 502 errors.
